...
const ProductBrand = ({setCurrPage, shop_right = false}) => {
   ...
   } else if (!isLoading && !isError && brands?.metadata?.thuong_hieus?.length === 0) {
      content = <ErrorMsg msg='No Brands found!' />;
   } else if (!isLoading && !isError && brands?.metadata?.thuong_hieus?.length > 0) {
      const all_brands = brands?.metadata?.thuong_hieus;
      const sortedBrands = all_brands.slice().sort((a, b) => b.created_at - a.created_at);
      const brand_items = sortedBrands.slice(0, 6);

      content = brand_items.map((b) => (
         <div key={b.ma_thuong_hieu} className='tp-shop-widget-brand-item'>
            <a onClick={() => handleBrandRoute(b.ma_thuong_hieu)} style={{cursor: "pointer"}}>
               <Image src={b.thumbnail_image} alt='brand' width={60} height={50} />
            </a>
         </div>
      ));
   }
