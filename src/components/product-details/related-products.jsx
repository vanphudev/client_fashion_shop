...
// internal
import {useGetProductLQQuery} from "@/redux/features/productApi";
...
const RelatedProducts = ({id}) => {
   const {data: products, isError, isLoading} = useGetProductLQQuery(id);
   ...
   if (!isLoading && !isError && products?.metadata?.san_phams?.length > 0) {
      const product_items = products?.metadata?.san_phams;
      content = (
         <Swiper
            {...slider_setting}
